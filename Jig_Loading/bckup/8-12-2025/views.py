...
class JigPickTableView(APIView):
    ...
    def get(self, request):
        ...
        recovery_stock_eligible = RecoveryStockModel.objects.filter(
            ...
            jig_hold_lot=True  # Only exclude "On-Hold" records
        ).select_related('batch_id')
        
        print(f"RecoveryStockModel eligible count: {recovery_stock_eligible.count()}")
        
        # STEP 2: Filter based on batch status - FIXED: Allow leftover rows ("Yet to Release")
        print("=== Filtering based on batch status - Allow leftover rows ===")
        
        total_stock_filtered = []
        for stock in total_stock_eligible:
            status = self.calculate_batch_status(stock.lot_id)
            # ✅ CRITICAL FIX: Allow "Yet to Release" (leftover rows) for multi-model jig loading
            # Only exclude truly unusable statuses
            if status['status'] not in ['Released']:  # Only exclude 'Released', allow 'Yet to Release'
        ...
        # STEP 6: Pagination - BUT allow bypassing for Add Model filtering
        page_number = request.GET.get('page', 1)
        
        # Check if this is an Add Model filter request (bypass pagination)
        is_add_model_filter = request.GET.get('add_model_filter', 'false').lower() == 'true'
        
        if is_add_model_filter:
            # For Add Model filtering, return ALL records to ensure leftover rows are included
            print("=== Add Model Filter: Returning ALL records (bypassing pagination) ===")
            page_obj = all_master_objects  # Return all records
            paginator = None
        else:
        ...
        if is_add_model_filter:
            # For Add Model filtering, process all records
            current_page_objects = all_master_objects
        else:
            # For regular pagination, process current page only
            current_page_objects = page_obj.object_list
        
        current_page_batch_ids = [obj.pk for obj in current_page_objects]
        
        # Get JigLoadingMaster data in bulk
        model_stock_nos = [obj.model_stock_no.pk for obj in current_page_objects if obj.model_stock_no]
        jig_loading_lookup = {}
        ...
        lot_ids = []
        for obj in current_page_objects:
            if isinstance(obj, ModelMasterCreation):
                stock = total_stock_lookup.get(obj.pk)
            else:  # RecoveryMasterCreation
                stock = recovery_stock_lookup.get(obj.pk)
        ...
        for jd in jig_details_qs:
            ...
            print(f"JigDetail: no_of_model_cases: {jd.no_of_model_cases} | remarks: '{jd.remarks}' | IP_loaded: {jd.IP_loaded_date_time}")
        
        # STEP 8: Build master_data efficiently
        master_data = []
        mode = "ALL RECORDS" if is_add_model_filter else "CURRENT PAGE"
        print(f"=== PROCESSING {len(current_page_objects)} RECORDS ({mode}) ===")
        
        for master_obj in current_page_objects:
            ...
            data['batch_status'] = batch_status
             ...
             # *** MODIFIED: Skip adding to master_data if status is 'Yet to Release' ***
            if batch_status['status'] == 'Yet to Release':
                continue
        ...
        # STEP 7: Get LookLikeModel data for frontend filtering
        lookalike_pairs = []
        try:
            from modelmasterapp.models import LookLikeModel
            for lookalike in LookLikeModel.objects.all():
                plating_stk_nos = [model.plating_stk_no for model in lookalike.plating_stk_no.all() if model.plating_stk_no]
                same_plating_stk_no = lookalike.same_plating_stk_no.plating_stk_no if lookalike.same_plating_stk_no and lookalike.same_plating_stk_no.plating_stk_no else None
                
                if plating_stk_nos and same_plating_stk_no:
                    for plating_stk_no in plating_stk_nos:
                        lookalike_pairs.append({
                            'model1': plating_stk_no,
                            'model2': same_plating_stk_no,
                            'description': lookalike.text
                        })
        except Exception as e:
            print(f"Error loading LookLikeModel data: {e}")
            lookalike_pairs = []
        
        print(f"LookLikeModel pairs loaded: {len(lookalike_pairs)}")
        
        context = {
            'master_data': master_data,
            'page_obj': page_obj if not is_add_model_filter else None,
            'all_data': master_data if is_add_model_filter else None,  # Include all data for filtering
            'paginator': paginator,
            'user': user,
            'is_admin': is_admin,
            'lookalike_model_pairs': lookalike_pairs,  # Add lookalike data
            'is_add_model_filter': is_add_model_filter,
        }
    ...
    def calculate_batch_status_efficient(self, stock_model):
        ...
        if jig_physical_qty > 0:
            return {
                'status': 'Partial Load',  # Changed from 'Draft' to 'Partial Load'
                ...
                if (draft.lot_id_quantities and 
                    ...
                    if len(updated_quantities) == 0:
                        ...
                        if not has_tray_work and not draft.jig_qr_id:
                            ...
                            # FIXED: Calculate total cases loaded based on actual jig capacity
                            # Use jig capacity if total quantities exceed it
                            total_qty = sum(updated_quantities.values())
                            if hasattr(draft, 'jig_capacity') and draft.jig_capacity:
                                draft.total_cases_loaded = min(total_qty, draft.jig_capacity)
                            else:
                                draft.total_cases_loaded = total_qty
...
def generate_optimal_tray_distribution_by_capacity(required_qty, tray_capacity, lot_id, available_tray_ids=None, force_original_qty=None):
    ...
    for i, (tray_id, actual_qty) in enumerate(actual_tray_data):
        ...
        trays_needed.append({
            'tray_id': tray_id,
            'tray_quantity': tray_capacity,
            'used_quantity': usable_from_tray,
            'original_tray_quantity': actual_qty,
            'lot_id': lot_id,
            'is_complete': usable_from_tray == tray_capacity,
            'is_top_tray': usable_from_tray < tray_capacity,
            'theoretical': True,
            'rejected_tray': False,
            'delink_tray': False,
        })
...
def calculate_half_filled_trays_by_capacity(stock_lot_id, display_qty, jig_capacity, tray_capacity, force_original_qty=None):
    ...
    - Only show half-filled if display_qty < original_qty
    - Show only the partial remainder (remaining_qty % tray_capacity)
    """
    half_filled_trays = []
    try:
        print(f"HALF FILLED CALCULATION (FINAL CORRECTED):")
        
        # Get true original quantity from DB
        stock_model, is_recovery, original_qty, _, _ = get_stock_model_data(stock_lot_id)
        print(f"  Stock type: {'Recovery' if is_recovery else 'Regular'}")
        print(f"  DB Original qty: {original_qty}")
        print(f"  Display qty: {display_qty}")
        print(f"  Tray capacity: {tray_capacity}")

        # FIXED: Only show half-filled if we're not processing the full original quantity
        if display_qty >= original_qty:
            print(f"  Processing full original qty ({display_qty} >= {original_qty}), no half-filled needed")
            return half_filled_trays

        # Calculate complete trays that will be used in delink for adjusted qty
        complete_trays_used = display_qty // tray_capacity
        actual_delink_qty = complete_trays_used * tray_capacity
        
        print(f"  Complete trays for delink: {complete_trays_used}")
        print(f"  Actual delink qty: {actual_delink_qty}")

        # FIXED: Calculate remaining based on jig capacity constraint
        # If display_qty is limited by jig capacity, the actual loaded is display_qty
        # The remaining is original_qty - display_qty (what couldn't fit in jig)
        actual_loaded_qty = min(display_qty, original_qty)
        remaining_qty = original_qty - actual_loaded_qty
        
        print(f"  Actual loaded qty: {actual_loaded_qty}")
        print(f"  Total remaining: {original_qty} - {actual_loaded_qty} = {remaining_qty}")
        
        if remaining_qty <= 0:
            print("  No remaining quantity, no half filled trays needed")
            return half_filled_trays

        # Calculate how many complete trays and partial remainder from the remaining
        complete_remaining_trays = remaining_qty // tray_capacity
        partial_remainder = remaining_qty % tray_capacity
        
        print(f"  Remaining breakdown: {complete_remaining_trays} complete + {partial_remainder} partial")
        
        # Only show partial remainder if it exists
        if partial_remainder > 0:
            half_filled_trays.append({
                'tray_id': f"REMAINING-PARTIAL",
                'tray_quantity': partial_remainder,
                'original_tray_quantity': tray_capacity,
                'is_top_tray': True,
                'lot_id': stock_lot_id,
                'theoretical': True
            })
            print(f"  ✓ HALF-FILLED TRAY: {partial_remainder} pieces (partial only)")
        else:
            print(f"  No partial remainder, no half-filled tray needed")
...
# Primary Function for Multi-Model 
def generate_multi_model_optimal_distribution(lot_quantities_dict, tray_capacity_dict, jig_capacity=0, broken_hooks=0):
    """
    ...
    try:
        ...
        total_requested = sum(lot_quantities_dict.values())
        print(f"Total requested: {total_requested} pieces")
        print(f"Jig capacity: {sum(tray_capacity_dict.values()) * 12} (estimated)")  # Rough estimate
        ...
        print(f"🔢 ADD MODEL COUNT: {len(lot_quantities_dict)} models added")
        print(f"📈 TOTAL REQUESTED (addition): {total_requested} pieces across all models")
        
        for lot_id, required_qty in lot_quantities_dict.items():
            if required_qty <= 0:
                continue
                
            print(f"ADD MODEL: Lot {lot_id} - Requested Quantity: {required_qty}")
            ...
            print(f"MODEL DETAILS: Lot {lot_id} - Original Qty: {original_qty}, Tray Capacity: {tray_capacity}, Is Recovery: {is_recovery}")
            ...
            print(f"PLATING STK NO: Lot {lot_id} - {plating_stk_no}")
            
            # Get JigLoadingMaster data for jig_capacity
            jig_master = None
            jig_capacity = 0
            try:
                ...
                print(f"Error getting jig_capacity for {lot_id}: {e}")
            
            print(f"JIG CAPACITY: Lot {lot_id} - {jig_capacity}")
            ...
            try:
                ...
                tray_count = actual_trays.count()
                total_tray_qty = sum(tray.tray_quantity for tray in actual_trays)
                print(f"TRAY DATA: Lot {lot_id} - {tray_count} trays, Total Quantity: {total_tray_qty}")
            ...
            all_lot_data[lot_id] = {
                ...
                'jig_capacity': jig_capacity  # Added jig_capacity here
            }
        ...
        all_trays.sort(key=lambda x: (list(lot_quantities_dict.keys()).index(x['lot_id']), x['priority']))
        ...
        print(f"Under-allocation expected: {total_required - total_available} pieces (by design)")
        
        # Step 3: Sequential allocation with jig capacity enforcement
        selected_trays = []
        total_allocated = 0
        lot_allocations = {lot_id: 0 for lot_id in lot_quantities_dict.keys()}
        lot_order = list(lot_quantities_dict.keys())
        
        # Get jig capacity from the first lot (they should all have the same jig capacity)
        jig_capacity = None
        for lot_data in all_lot_data.values():
            if lot_data.get('jig_capacity'):
                jig_capacity = lot_data['jig_capacity']
                break
        
        if jig_capacity is None:
            jig_capacity = 98  # Default fallback
            print(f"⚠️ No jig capacity found, using default: {jig_capacity}")
        ...
        for lot_id in lot_order:
            required = lot_quantities_dict[lot_id]
            allocated = 0
            
            print(f"ALLOCATION START: Lot {lot_id} - Required: {required}, Remaining Jig Capacity: {remaining_jig}")
            
            # Get trays for this lot from all_trays
            lot_trays = [t for t in all_trays if t['lot_id'] == lot_id]
            # Sort by priority (partials first)
            lot_trays.sort(key=lambda x: x['priority'])
            
            print(f"TRAYS FOR LOT {lot_id}: {len(lot_trays)} trays available")
            
            for tray in lot_trays:
                if remaining_jig <= 0:
                    print(f"JIG CAPACITY EXHAUSTED: Stopping allocation for Lot {lot_id}")
                    break
                
                usable = min(tray['usable_qty'], remaining_jig)
                
                # Allocate this tray
                tray['allocated_qty'] = usable
                selected_trays.append(tray)
                allocated += usable
                remaining_jig -= usable
                total_allocated += usable
                
                status = "partial" if tray['is_partial'] else "complete"
                print(f"TRAY ALLOCATION: {tray['tray_id']} ({tray['lot_id']}) - Used: {usable}/{tray['tray_capacity']} pieces ({status})")
                print(f"ALLOCATION PROGRESS: Lot {lot_id} - Total Allocated: {allocated}, Jig Remaining: {remaining_jig}")
            
            # If no trays were allocated for this lot but we have remaining jig capacity, simulate allocation
            if allocated == 0 and remaining_jig > 0:
                # Simulate allocation based on required quantity limited by remaining jig
                simulated_allocated = min(required, remaining_jig)
                allocated = simulated_allocated
                remaining_jig -= simulated_allocated
                total_allocated += simulated_allocated
                print(f"SIMULATED ALLOCATION: Lot {lot_id} - Allocated: {simulated_allocated} pieces (no actual trays)")
            
            lot_allocations[lot_id] = allocated
            deduction = required - allocated
            print(f"ALLOCATION COMPLETE: Lot {lot_id} - Requested: {required}, Allocated: {allocated}, Deduction (Not Allocated): {deduction}")
        
        print(f"Sequential allocation complete: {total_allocated} pieces from {len(selected_trays)} trays")
        ...
        for lot_id, lot_data in all_lot_data.items():
            ...
            print(f"  - Required Quantity: {original_required}")
            print(f"  - Allocated Quantity: {lot_allocated}")
            print(f"  - Delink Trays: {len(lot_trays)}")
            print(f"  - Remaining Pieces: {original_required - lot_allocated}")
            ...
            result['lot_distributions'][lot_id] = {
                ...
                'half_filled_trays': lot_half_filled,  # Only last lot gets half-filled
...
def get_multi_model_distribution(request):
    ...
    try:
        ...
        # If tray_capacity_dict not provided, derive capacities per lot (backwards-compatible)
        if not tray_capacity_dict:
            tray_capacity_dict = {}
    ...
    try:
        ...
        try:
            ...
            if draft_jig:
                ...
                # *** NEW: If draft has no half_filled_tray_data, calculate it based on remaining qty ***
                if not formatted_draft_half_filled:
                    remaining_qty_for_draft = accepted_qty - draft_jig.total_cases_loaded
                    calculated_half_filled = calculate_half_filled_trays_by_capacity(
                        stock_lot_id, 
                        remaining_qty_for_draft, 
                        jig_capacity, 
                        tray_capacity,
                        force_original_qty=force_qty
                    )
                    formatted_draft_half_filled = calculated_half_filled
                    print(f"✅ Calculated half_filled_trays for draft: {len(formatted_draft_half_filled)} items")
                ...
                # Always use the draft (or calculated) half_filled_tray_data for consistency
                half_filled_trays = formatted_draft_half_filled
...
class JigDetailsSaveAPIView(APIView):
    ...
    def calculate_actual_lot_quantities(self, lot_id_quantities, total_cases_loaded, faulty_slots, empty_slots, primary_lot_id, is_draft):
        ...
        # For multi-model, use original quantities without capping
        if len(lot_id_quantities) > 1:
            print(f"📊 Multi-model detected, using original quantities without capping")
    ...
    def post(self, request):
        ...
        try:
            ...
            # Calculate jig_cases_remaining_count based on requested quantities (not capped allocated)
            jig_cases_remaining_count = max(0, total_requested - jig_master.jig_capacity)
...
class JigDetailsClearDraftAPIView(APIView):
    def post(self, request):
        try:
            data = request.data if hasattr(request, 'data') else json.loads(request.body.decode('utf-8'))
            lot_id = data.get('lot_id', '').strip()
            print(f"Received lot_id for draft clear: {lot_id}")  # <-- Add this line
            if not lot_id:
                return JsonResponse({'success': False, 'error': 'Missing lot_id'}, status=400)
            
            deleted_count, _ = JigDetails.objects.filter(lot_id=lot_id, draft_save=True).delete()
            if deleted_count > 0:
                return JsonResponse({'success': True, 'message': f'Draft(s) for lot_id {lot_id} cleared.'})
            else:
...
def validate_tray_id(request):
    ...
    lot_id = data.get('lot_id', '').strip()
    is_half_filled = data.get('is_half_filled', False)
    ...
    # and not delinked). This prevents accidentally reusing occupied trays.
    if is_half_filled:
        ...
        if occupied:
            # Treat this as invalid for half-filled usage
            return JsonResponse({'exists': False, 'lot_match': False, 'error': 'Tray is occupied with counts'})
...
class JigDetailsValidateQRAPIView(APIView):
    """
    API endpoint for validating JIG QR ID - must exist in system
    """
    def post(self, request):
        try:
            data = request.data if hasattr(request, 'data') else json.loads(request.body.decode('utf-8'))
            ...
            # Check if JIG QR ID exists in the database
            exists = Jig.objects.filter(jig_qr_id=jig_qr_id).exists()
            ...
            if exists:
                ...
                return JsonResponse({
                    'success': True,
                    'exists': True,
                    'message': f'JIG QR ID "{jig_qr_id}" is valid'
                })
            else:
                # JIG doesn't exist - ERROR
                return JsonResponse({
                    'success': False,
                    'exists': False,
                    'message': f'JIG QR ID "{jig_qr_id}" not found. Please scan a valid JIG QR code.'
                })
...
def get_jig_autosave(request, lot_id):
    try:
        autosave = JigAutoSave.objects.get(user=request.user, lot_id=lot_id)
        if autosave.is_expired():
            autosave.delete()
            return JsonResponse({'success': True, 'data': None})
        return JsonResponse({'success': True, 'data': autosave.to_dict()})
    except JigAutoSave.DoesNotExist:
        return JsonResponse({'success': True, 'data': None})
