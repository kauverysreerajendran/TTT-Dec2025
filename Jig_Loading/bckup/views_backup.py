...
def validate_jig_id_and_type(request):
    """
    Validate if the scanned Jig ID exists and has the correct Jig Type
    for the selected model/lot based on JigLoadingMaster configuration
    """
    import json
    try:
        data = json.loads(request.body.decode('utf-8'))
        ...
        jig_obj = Jig.objects.filter(jig_qr_id=jig_qr_id).first()
        if not jig_obj:
            return JsonResponse({'valid': False, 'error': f'Jig ID "{jig_qr_id}" not found in Jig master table.'})
        ...
        if not stock:
            return JsonResponse({'valid': False, 'error': f'Lot ID "{lot_id}" not found in stock.'})
        ...
        if hasattr(stock, 'batch_id') and stock.batch_id:
            ...
            return JsonResponse({'valid': False, 'error': 'Model stock number not found for this lot.'})
        
        # Find the required jig type from JigLoadingMaster
        jig_master = JigLoadingMaster.objects.filter(model_stock_no=model_stock_no).first()
        if not jig_master:
            return JsonResponse({'valid': False, 'error': f'Jig configuration not found for model "{model_stock_no}". Please contact admin.'})
        
        expected_jig_type = jig_master.jig_type
        
        # Check if scanned jig has the correct type
        if not jig_obj.jig_type:
            return JsonResponse({'valid': False, 'error': f'Jig ID "{jig_qr_id}" does not have jig type configured. Please contact admin.'})
        
        if jig_obj.jig_type != expected_jig_type:
            return JsonResponse({
                'valid': False, 
                'error': f'Jig type mismatch! Expected "{expected_jig_type}" for model "{model_stock_no}", but scanned jig "{jig_qr_id}" is type "{jig_obj.jig_type}".'
            })

        # All validations passed
        return JsonResponse({
            'valid': True, 
            'message': f'Jig ID "{jig_qr_id}" is valid. Type "{jig_obj.jig_type}" matches requirement.',
            'jig_type': jig_obj.jig_type,
