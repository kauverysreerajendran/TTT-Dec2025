...
# Half Filled Tray Logic for Multi-Model
...
def fetch_jig_related_data(request):
    ...
    try:
        ...
        #  Loaded Case Qty calculation 00/00 ---
        scanned_qty = JigLoadTrayId.objects.filter(
            lot_id=stock_lot_id,
            rejected_tray=False,
            delink_tray=False,
            tray_id__isnull=False
        ).aggregate(total=models.Sum('tray_quantity'))['total'] or 0
        loaded_case_qty = f"{scanned_qty}/{display_qty}"
        ...
        return Response({
            ...
            'loaded_case_qty': loaded_case_qty,
        })
...
@api_view(['GET', 'POST'])
def get_scanned_tray_count(request):
    """
    Calculate and return the current scanned tray count for a lot_id
    Dynamic scanning count: scanned_qty/total_qty
    Now supports tracking actual scanned trays in current session
    """
    if request.method == 'GET':
        lot_id = request.GET.get('lot_id', '').strip()
        scanned_tray_ids = request.GET.get('scanned_tray_ids', '').strip()
    else:  # POST
        data = request.data if hasattr(request, 'data') else json.loads(request.body.decode('utf-8') or '{}')
        lot_id = data.get('lot_id', '').strip()
        scanned_tray_ids = data.get('scanned_tray_ids', '').strip()
    
    if not lot_id:
        return JsonResponse({'error': 'lot_id is required'}, status=400)
    
    try:
        # Get the total target quantity for this lot
        # Try TotalStockModel first
        stock_model = TotalStockModel.objects.filter(lot_id=lot_id).first()
        if not stock_model:
            # Try RecoveryStockModel
            stock_model = RecoveryStockModel.objects.filter(lot_id=lot_id).first()
        
        if not stock_model:
            return JsonResponse({'error': f'No stock data found for lot_id: {lot_id}'}, status=404)
        
        # Get model master for jig capacity
        batch = stock_model.batch_id if hasattr(stock_model, 'batch_id') and stock_model.batch_id else None
        jig_capacity = 0
        if batch:
            jig_master = JigLoadingMaster.objects.filter(
                model_stock_no=batch.model_stock_no
            ).first() if hasattr(batch, 'model_stock_no') else None
            jig_capacity = jig_master.jig_capacity if jig_master else 0
        
        # Calculate the target quantity (quantity to be processed in current jig)
        if hasattr(stock_model, 'jig_physical_qty') and stock_model.jig_physical_qty:
            base_qty = stock_model.jig_physical_qty
        else:
            base_qty = getattr(stock_model, 'brass_audit_accepted_qty', 0) or 0
        
        # Cap by jig capacity
        target_qty = min(base_qty, jig_capacity) if jig_capacity > 0 else base_qty
        
        # Calculate scanned quantity based on provided scanned tray IDs
        scanned_qty = 0
        if scanned_tray_ids:
            # Parse scanned tray IDs (comma-separated)
            tray_id_list = [tid.strip() for tid in scanned_tray_ids.split(',') if tid.strip()]
            
            if tray_id_list:
                # Get quantities for these specific tray IDs
                scanned_trays = JigLoadTrayId.objects.filter(
                    lot_id=lot_id,
                    tray_id__in=tray_id_list,
                    rejected_tray=False,
                    delink_tray=False
                )
                
                scanned_qty = scanned_trays.aggregate(
                    total=models.Sum('tray_quantity')
                )['total'] or 0
                
                print(f"🔢 Calculated scanned_qty for tray_ids {tray_id_list}: {scanned_qty}")
        ...
        return JsonResponse({
            ...
            'display_text': f"{scanned_qty}/{target_qty}",
            'percentage': round((scanned_qty / target_qty * 100), 1) if target_qty > 0 else 0,
            'scanned_tray_ids': scanned_tray_ids
        })
        
    except Exception as e:
        print(f"Error in get_scanned_tray_count: {str(e)}")
    ...
    def post(self, request):
        try:
            ...
            if not jig_obj:
                ...
                return JsonResponse({'valid': False, 'error': '', 'jig_qr_id': jig_qr_id}, status=404)
...
def validate_jig_id_and_type(request):
    ...
    try:
        ...
        # Extract jig type from Jig ID (keep leading zeros)
        try:
            jig_type_from_id = jig_qr_id.split('-')[0]  # e.g. "J098" from "J098-0001"
