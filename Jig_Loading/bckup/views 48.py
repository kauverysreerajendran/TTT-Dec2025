...
file_handler = logging.FileHandler(r"F:\New_TTT_Sep\Jig_Loading\jig_loading.log")
...
def validate_jig_qr(request):
    # Debug: Log request data (avoid accessing request.body after DRF parsing)
    logger.info(f"🔍 validate_jig_qr function called - Method: {request.method}")
    logger.info(f"🔍 Request data: {getattr(request, 'data', {})}")
    
    jig_qr_id = (request.data.get('jig_qr_id') or '').strip()
    # NEW: optional lot_id provided by frontend to persist autosave
    lot_id = (request.data.get('lot_id') or '').strip()

    logger.info(f"🔍 validate_jig_qr called: jig_qr_id='{jig_qr_id}', lot_id='{lot_id}'")

    if not jig_qr_id:
        return JsonResponse({'valid': False, 'error': 'JIG QR ID required'})

    # read-only check — do NOT create object here
    try:
        ...
        if jig_obj:
            ...
            # Valid and available — attempt autosave if lot_id provided
            valid_resp = {
                'valid': True,
            ...
            # Not found - treat as available for new creation on submit,
            # but do not create the Jig object here.
            valid_resp = {
                'valid': True,
                'message': f'JIG QR ID {jig_qr_id} is valid and available',
                'jig_qr_id': jig_qr_id
        ...
        # NEW: Persist into JigAutoSave if frontend sent a lot_id
        if lot_id:
            logger.info(f"🔄 Attempting to autosave jig_qr_id='{jig_qr_id}' for lot_id='{lot_id}'")
            try:
                ...
                if user:
                    # user autosave (keep session_key blank for user-owned rows)
                    autosave, created = JigAutoSave.objects.update_or_create(
                        user=user,
                        lot_id=lot_id,
                        defaults={**defaults, 'session_key': ''}
                    )
                    logger.info(f"✅ User autosave {'created' if created else 'updated'}: user={user.username}, lot_id={lot_id}")
                else:
                    # anonymous autosave by session_key
                    autosave, created = JigAutoSave.objects.update_or_create(
                        session_key=session_key,
                        lot_id=lot_id,
                        defaults={**defaults, 'user': None}
                    )
                    logger.info(f"✅ Session autosave {'created' if created else 'updated'}: session_key={session_key}, lot_id={lot_id}")
            except Exception as save_err:
                # Log and continue returning valid=True (non-blocking)
                logger.exception("❌ Error saving jig autosave: %s", str(save_err))
        else:
            logger.info("ℹ️  No lot_id provided, skipping autosave")
