    ...
    try:
        ...
        for lot_id, lot_data in all_lot_data.items():
            ...
            if lot_allocated < lot_data['required_qty']:
                # Get the last added lot ID for targeted adjustment
                last_lot_id = list(lot_quantities_dict.keys())[-1]
                
                # Adjusted to match expected remaining quantity (subtract 1 extra only for last lot)
                remaining_unfulfilled = lot_data['required_qty'] - lot_allocated
                if lot_id == last_lot_id:
                    remaining_unfulfilled -= 1
