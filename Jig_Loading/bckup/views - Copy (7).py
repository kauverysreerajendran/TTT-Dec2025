...
def generate_multi_model_optimal_distribution(lot_quantities_dict, tray_capacity_dict):
    ...
    try:
        ...
        print(f"Total requested: {sum(lot_quantities_dict.values())} pieces")
        ...
        print(f"🎯 Jig capacity: {jig_capacity}, Total required: {total_required}")
        
        remaining_jig = jig_capacity
        ...
        for lot_id in lot_order:
            ...
            for tray in lot_trays:
                ...
                print(f"✓ Sequential allocate: {tray['tray_id']} ({tray['lot_id']}) using {usable}/{tray['tray_capacity']} pieces ({status})")
            ...
            if allocated == 0 and remaining_jig > 0:
                ...
                print(f"✓ Simulated allocate: {lot_id} using {simulated_allocated} pieces (no actual trays)")
            
            lot_allocations[lot_id] = allocated
            
            # If this is the last lot and it overflows, create half-filled tray
            if lot_id == lot_order[-1] and allocated < required:
                # Find the last tray allocated for this lot
                last_tray_for_lot = None
                for tray in reversed(selected_trays):
                    if tray['lot_id'] == lot_id:
                        last_tray_for_lot = tray
                        break
                
                if last_tray_for_lot:
                    # half_qty = last_tray_for_lot['allocated_qty']
                    half_qty = all_lot_data[lot_id]['tray_capacity'] - last_tray_for_lot['allocated_qty']
                    

                else:
                    half_qty = required - allocated
                
                half_filled_tray = {
                    'tray_id': f"{all_lot_data[lot_id]['plating_stk_no']}-REMAINING",
                    'tray_quantity': half_qty,
                    'original_tray_quantity': all_lot_data[lot_id]['tray_capacity'],
                    'is_top_tray': True,
                    'lot_id': lot_id,
                    'plating_stk_no': all_lot_data[lot_id]['plating_stk_no'],
                    'theoretical': True,
                    'is_multi_model': True,
                    'verification_required': True
                }
                result['half_filled_trays'].append(half_filled_tray)
                print(f"✓ Half-filled tray created for last lot {lot_id}: {half_qty} pieces")
