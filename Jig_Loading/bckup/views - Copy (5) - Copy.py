...
def generate_multi_model_optimal_distribution(lot_quantities_dict, tray_capacity_dict):
    ...
    try:
            ...
            if not tray['is_partial']:
                ...
                if tray['usable_qty'] > lot_remaining:
                    ...
                    if waste_ratio > 0.25:  # Don't waste >25% of a complete tray
                        # *** FIX: Don't skip for the original lot (first in lot_quantities_dict) ***
                        original_lot_id = list(lot_quantities_dict.keys())[0]
                        if tray['lot_id'] != original_lot_id:
                ...
                if global_efficiency < 0.6:  # Don't use complete tray if we need <60% globally
                    # *** FIX: Don't skip for the original lot ***
                    original_lot_id = list(lot_quantities_dict.keys())[0]
                    if tray['lot_id'] != original_lot_id:
        ...
        for lot_id, lot_data in all_lot_data.items():
            ...
            # Calculate half-filled based on REQUIRED vs ALLOCATED (proper remaining calculation)
            lot_half_filled = []
            if lot_allocated < lot_data['required_qty']:
                # Calculate actual remaining quantity that needs verification
                ...
                # Only create half-filled entry if there's actually remaining quantity
                if remaining_unfulfilled > 0:
                    ...
                    print(f"✓ Half-filled tray created: {remaining_unfulfilled} pieces for verification")
            else:
        ...
        # *** FIX: Only show half-filled trays for the LAST ADDED LOT ***
        if lot_quantities_dict:
            last_lot_id = list(lot_quantities_dict.keys())[-1]  # Last added lot (dict preserves order)
            result['half_filled_trays'] = [
                tray for tray in result['half_filled_trays'] 
                if tray.get('lot_id') == last_lot_id
            ]
            print(f"✅ Filtered half-filled trays to only last added lot: {last_lot_id}")
...
class JigDetailsSaveAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            jig_cases_remaining_count = max(0, jig_master.jig_capacity - actual_total_cases_loaded - faulty_slots)
